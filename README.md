...
- The mismatch guard's category cross-check is synonym/word-boundary
  ...
  learned classifier — appropriate for this bounded corpus, not a
...
- `moondream` occasionally returns an empty caption for a given
  ...
  with alternate-prompt retries in `classifyImage.js`, but the underlying
  model behavior is a known rough edge, documented in `BUILDLOG.md`.
